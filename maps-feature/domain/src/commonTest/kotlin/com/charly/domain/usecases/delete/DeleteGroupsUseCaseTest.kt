package com.charly.domain.usecases.delete

import com.charly.domain.repositories.database.delete.DeleteGroupsRepository
import com.charly.domain.usecases.database.delete.DeleteGroupsUseCase
import dev.mokkery.answering.returns
import dev.mokkery.answering.throws
import dev.mokkery.everySuspend
import dev.mokkery.mock
import dev.mokkery.verify.VerifyMode
import dev.mokkery.verifySuspend
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.ExperimentalCoroutinesApi
import kotlinx.coroutines.test.StandardTestDispatcher
import kotlinx.coroutines.test.resetMain
import kotlinx.coroutines.test.runTest
import kotlinx.coroutines.test.setMain
import kotlin.test.AfterTest
import kotlin.test.BeforeTest
import kotlin.test.Test
import kotlin.test.assertFailsWith
import kotlin.test.assertSame

@OptIn(ExperimentalCoroutinesApi::class)
class DeleteGroupsUseCaseTest {

    @BeforeTest
    fun setUp() {
        Dispatchers.setMain(StandardTestDispatcher())
    }

    @AfterTest
    fun tearDown() {
        Dispatchers.resetMain()
    }

    @Test
    fun `Verify that execute succeeds`() = runTest {
        // GIVEN
        val groupIdsSelected = setOf(1L, 2L)
        val deleteGroupsRepository = mock<DeleteGroupsRepository>() {
            everySuspend { execute(groupIdsSelected) } returns Unit
        }

        val deleteGroupsUseCase = DeleteGroupsUseCase(deleteGroupsRepository)

        // WHEN
        deleteGroupsUseCase.execute(groupIdsSelected)

        // THEN
        verifySuspend(mode = VerifyMode.Companion.exhaustiveOrder) {
            deleteGroupsRepository.execute(groupIdsSelected)
        }
    }

    @Test
    fun `Verify that execute fails`() = runTest {
        // GIVEN
        val expectedException = Exception("Error")
        val groupIdsSelected = setOf(1L, 2L)
        val deleteGroupsRepository = mock<DeleteGroupsRepository>() {
            everySuspend { execute(groupIdsSelected) } throws expectedException
        }
        val deleteGroupsUseCase = DeleteGroupsUseCase(deleteGroupsRepository)

        // WHEN
        val actualException = assertFailsWith<Exception> {
            deleteGroupsUseCase.execute(groupIdsSelected)
        }

        // THEN
        assertSame(expectedException, actualException)
        verifySuspend(mode = VerifyMode.Companion.exhaustiveOrder) {
            deleteGroupsRepository.execute(groupIdsSelected)
        }
    }
}
